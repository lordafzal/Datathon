            ...
            try {
                switch (args[0]) {
                    case "encrypt":
                        ...
                        String key = stringFromFile(args[2]);
                        printCastle();
                        for (int i = 3; i < args.length; i++) {
                            String fileName = args[i];
                            System.out.println("Encrypting...  " + fileName);
                            encryptFile(fileName, key, prefix + fileName);
                        }
                        break;
                    case "decrypt":
                        String prefix2 = args[1];
                        String key2 = stringFromFile(args[2]);
                        printLock();
                        for (int i = 3; i < args.length; i++) {
                            String fileName = args[i];
                            System.out.println("Decrypting...  " + fileName);
                            String out = fileName.replace(prefix2, "");
                            decryptFile(fileName, key2, out);
                        }
                        break;
                    case "genKey":
                        genKey(args[1]);printKey();
                        break;
                }
            } catch (IOException e) {
                e.printStackTrace();
